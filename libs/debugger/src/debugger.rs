...
fn mutate(threadid: i32, corpus: Arc<Mutex<Corpus>>) -> String{
    
    let mut corpus = corpus.lock().unwrap();
    let x = corpus.mutate_rand(threadid);
    drop(corpus);
    x
...
pub struct Debugger<'a> {
    ...
    /// 
    /// we can technically make this public, to restore the same breakpoints, but idfk if theres a point
    pub target_breakpoints: HashMap<String, Vec<Breakpoint>>,
    ...
    pub minmax_breakpoint: HashMap<String, (usize, usize)>,
    ...
    pub had_new_coverage: bool, 
}


fn stats(iterations: u64){
    print!("iterations: {}\n", iterations);
}
...
impl<'a> Debugger<'a> {
    ...
    pub fn ifnewcov(&self) -> bool{
        self.had_new_coverage
    }
    ...
    /// Handle a breakpoint
    fn handle_breakpoint(&mut self, tid: u32, addr: usize, case_end: bool) -> bool {
        if let Some(ref mut bp) = self.breakpoints.get_mut(&addr).cloned() {
            ...
            // FIGURE OUT HOW TO PLACE A BREAKPOINT AFTER RUNNING THE INSTRUCTION
            //if !case_end {
    ...
    pub fn run(&mut self,corpus: Arc<Mutex<Corpus>>) -> ExitType {

        //print!("ezzzzz");
        let windowname = "Untitled - Notepad";
        //std::thread::sleep_ms(25000);

        let mut window:Arc<window::Window> = Arc::new(window::Window::new());

        let exit_module_name = "notepad.exe";
        let mut exit_module_base = 0;
        let case_end_address: usize = 0xe601;

        let mut event = unsafe { std::mem::zeroed() };
        let mut  iteration: u64 = 0;
        let mut hit_initial_break = false;
        let mut case_end_addy: usize =  0;

        let mut last_was_the_end = false;
        unsafe { loop {
            let mut thiswastheend = false;
            ...
            match event.dwDebugEventCode {
                ...
                EXCEPTION_DEBUG_EVENT => {
                    ...
                    if exception.ExceptionRecord.ExceptionCode == 0x80000003 {
                        ...
                        //print!("we hit this {:x}\n", (exception.ExceptionRecord.ExceptionAddress as usize)-(exit_module_base));
                        if(0xa3bd == (exception.ExceptionRecord.ExceptionAddress as usize)-(exit_module_base)){
                            print!("we hit it {:x}\n", (exception.ExceptionRecord.ExceptionAddress as usize)-(exit_module_base));
                            thiswastheend = true;
                            last_was_the_end = true;
                            if window.hwnd != 0{

                                fuzz(corpus.clone(), iteration, window.clone());
                                iteration = iteration+1;
                                let corpus = corpus.lock().unwrap();
                                print!("files: {}\n", corpus.filenames.len() );
                            }
                        }
                        ...
                        if exit_module_base == 0 {
                            for (module, base) in self.modules.iter(){
                                if module == exit_module_name{
                                    exit_module_base = *base;
                                }
                            }
                        }
                
                        if (exception.ExceptionRecord.ExceptionAddress as usize)-exit_module_base == case_end_address {
                            print!("we hit it {:x}\n", (exception.ExceptionRecord.ExceptionAddress as usize)-(exit_module_base));
                            
                            case_end_addy = exception.ExceptionRecord.ExceptionAddress as usize;
                            thiswastheend = true;
                            last_was_the_end = true;
                            {
                                let mut corpus = corpus.lock().unwrap();
                                print!("removing {}\n", corpus.lastinputfn());
                                std::fs::remove_file(corpus.lastinputfn()).expect("bruh");
                            }
                            if window.hwnd != 0{
                        ...
                        if !self.handle_breakpoint(tid,
                                ...
                                .ExceptionAddress as usize, thiswastheend) {
                            //mprint!(self, 
                            //    "Warning: Continuing unexpected 0x80000003\n");
                        }
                        if self.had_new_coverage && thiswastheend{
                        ...
                        /*
                        let case_end_address: usize = 0xacdd;

                        if exception.ExceptionRecord.ExceptionAddress as usize == case_end_address{
                            return ExitType::ExitCode(696969);
                        }*/ 
                    } else {
            ...
            if thiswastheend == false && last_was_the_end == true{
                self.write_mem(case_end_addy, &[0xcc]);
                last_was_the_end = false;
            }
            if window.hwnd == 0 {
                //print!("no window");
                window = Arc::new(window::Window::get_window(windowname));
            }
            if(window::ifexit()){
                return ExitType::ExitCode(6000);
            }
            assert!(ContinueDebugEvent(
                    pid, tid, DBG_CONTINUE) != 0);
...
fn fuzz(corpus: Arc<Mutex<Corpus>>, iteration: u64, window: Arc<window::Window>){
    
    {
        ...
        std::thread::spawn(move || {
            ...
            let fname: String = mutate(0, corpus.clone());
            window.fuzz_case(&fname);
            let mut corpus = corpus.lock().unwrap();
            corpus.setlastinputfn(fname);
        });
    }
}
fn close_case(window: Arc<window::Window>){
    {
        std::thread::spawn(move || {
            window.close_case();
        });
